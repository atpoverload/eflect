...
import eflect.protos.sample.StartResponse;
...
public final class SamplerServer {
  static class SamplerImpl extends SamplerGrpc.SamplerImplBase {
    @Override
    public void start(SamplerRequest req, StreamObserver<SamplerReply> responseObserver) {
      StartResponse response = StartResponse.getDefaultInstance();
  ...
  //       sampleFutures.add(
  //           executor.schedule(
  //               () -> collectAndReschedule(source, period), rescheduleTime.toNanos(),
  // NANOSECONDS));
  //     } else {
