...
def account_jiffies(app, cpu, *args):
    # accounted = (app / cpu.sum(axis = 1)).replace(np.inf, 1).clip(0, 1)
    df = pd.concat([app, cpu.sum(axis = 1)], axis = 1).sort_values(by = ['timestamp', 'run', 'domain'])
    df.columns = ['app', 'total']
    return df

def account_application_energy(app, cpu, energy):
    accounted = energy * (app / cpu.sum(axis = 1)).replace(np.inf, 1).clip(0, 1)
    df = pd.concat([accounted, energy], axis = 1).sort_values(by = ['timestamp', 'run', 'domain'])
    df.columns = ['app', 'total']
    return df
