...
public class LabelImage {
  ...
  public static void main(String[] args) {
    ...
    // "/home/timur/projects/tensorflow/bazel-bin/tensorflow/java/libtensorflow_jni.so");
    if (args.length != 2) {
    ...
    try (Tensor<Float> image = constructAndExecuteGraphToNormalizeImage(imageBytes)) {
      ...
      // read with proc
      EflectCalmnessMonitor.getInstance().start(128);
      for (int i = 0; i < 250; i++) {
        float[] labelProbabilities = executeInceptionGraph(graphDef, image);
        if (i > 0 && (i + 1) % 25 == 0) {
          System.out.println("iter " + (i + 1) + " at " + Instant.now());
        }
      }
